...
const { promptPaymentAuth } = __dep2;
const { getUserInfo } = __dep3;
...
export default {
    ...
    async handleNewSession() {
        ...
        this.setData({ loading: true });
        uni.showLoading({ title: '新建会话中...', mask: true });
        try {
            const res = await createConversation({ title: '新会话' });
            ...
            await this.loadHistory(activeId);
            uni.showToast({ title: '已新建对话', icon: 'success' });
        } catch (e) {
            console.error(e);
            uni.showToast({ title: '新建对话失败', icon: 'none' });
        } finally {
            this.setData({ loading: false });
            uni.hideLoading();
        }
        ...
        try {
            ...
            const messages = (list || []).map((item) => {
                ...
                return {
                    ...
                    action_resolved: item.action_resolved || null,
    ...
    async handleApprove(e) {
        ...
        try {
            ...
            uni.showLoading({ title: '正在执行...', mask: true });
            ...
            uni.showToast({ title: '操作已授权执行', icon: 'success' });
    ...
    async handleReject(e) {
        ...
        try {
            ...
            uni.showToast({ title: '已拒绝该操作', icon: 'none' });
            await this.loadHistory(this.data.conversationId);
        } catch (err) {
            uni.hideLoading();
            uni.showToast({
